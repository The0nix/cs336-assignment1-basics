...
def _get_most_common_pair(pair_counts: Counter[tuple[bytes, bytes]]) -> tuple[bytes, bytes]:
...
def train_bpe_tokenizer(
    ...
    while len(vocab) < vocab_size:
        ...
        try:
            most_common_pair = _get_most_common_pair(pair_counts)
        ...
        for word in list(word_counts):
            ...
            while i < len(word) - 1:
                if (word[i], word[i + 1]) == most_common_pair:
                    if i > 0:
                        pair_counts[(word[i - 1], word[i])] -= word_counts[word]
                        pair_counts[(word[i - 1], new_bytes)] += word_counts[word]
                    if i < len(word) - 2:
                        pair_counts[(word[i + 1], word[i + 2])] -= word_counts[word]
                        pair_counts[(new_bytes, word[i + 2])] += word_counts[word]
                    new_word.append(new_bytes)
                    i += 2
                else:
                    new_word.append(word[i])
                    i += 1
            if len(word) == 1 or (word[-2], word[-1]) != most_common_pair:
